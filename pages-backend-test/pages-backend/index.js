  ...
  const form = new formidable.IncomingForm();
    ...
    form.parse(req, async function (err, fields, files) {
      try {
        ...
        return res.status(200).json({ message: 'Microservice deleted successfully.' });
      } catch (err) {
          console.error(err);
          return res.status(500).json({ error: 'An error occurred while deleting the microservice.' });
      }
      })
    })
  ...
  try {
    ...
    form.parse(req, async function (err, fields, files) {
      ...
      if (version) {
        ...
        res.status(200).json({ message: 'Version deleted successfully.', name: microservice.svcname});
      } else {
        console.log('No version found for the microservice.');
        res.status(404).json({ message: 'No version found for the microservice.' });
      }
    });
  } catch (err) {
    res.status(400).json({ error: err })
...
app.post("/start", async(req, res) => {
  try {
    ...
    let parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/serverstart");

    let child = spawn("./serverstart",[8888, 8889, 8890], { cwd: parentDir });
    ...
    try {
      ...
      let parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/nstart");
      ...
      if (osType === 'win32') {
          child = spawn("./nstart.exe", { cwd: parentDir });
      } else {
          child = spawn("./nstartmac", { cwd: parentDir });
      }
...
app.post("/stop", async(req, res) => {
  try {
    const parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/shutdown");
    ...
    console.log("os is " + osType)
    if (osType === 'win32') {
        child = spawn("./shutdown.exe", [8888, 8889, 8890], { cwd: parentDir });
    } else {
        child = spawn("./shutdown", [8888, 8889, 8890], { cwd: parentDir });
    }
    ...
    child.on('close', (code) => {
        if(code == 0) {
          ...
          res.status(200).json({outcome: 'Error Stopping Servers'})
          return
        }
      })
    } catch (err) {
      res.status(100).json({outcome: "Error Stopping Gateway"})
      return
    }

    try {
      const parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/nstop");
      ...
      console.log("os is " + osType)
      if (osType === 'win32') {
          child = spawn("./nstop.exe", { cwd: parentDir });
      } else {
          child = spawn("./nstopmac", { cwd: parentDir });
      }
      ...
      child.on('close', (code) => {
          if(code == 0) {
            ...
            res.status(200).json({outcome: 'Error Stopping Gateway'})
          }
        })
      } catch (err) {
        res.status(100).json({outcome: "Error Stopping Gateway"})
...
app.post("/update", async (req, res) => {
  try {
    ...
    form.parse(req, async function (err, fields) {
      ...
      const { url, lb } = fields;
      const parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/update");


      const child = spawn("./update", ["any", lb], { cwd: parentDir });
      ...
      child.on('close', (code) => {
          if(code == 0) {
            ...
            console.log("Error Updated Gateway")
            res.status(200).json({outcome: 'Error Updated Gateway'})
          }
        })
      })
 } catch (err) {
...
app.post("/gen", async (req, res) => {
  try {
    ...
    form.parse(req, async function (err, fields) {
      ...
      const child = spawn('./gen', [url, "start", lb], { cwd: parentDir });
      ...
      child.on('close', (code) => {
          if(code == 0) {
            ...
            console.log("Error generating Gateway")
            res.status(200).json({outcome: 'Error Generating Gateway'})
          }
      });
...
app.post("/del", async (req, res) => {
  try {
    const parentDir = path.join(__dirname, '..', "..","/JSONGenwithpostres/del");
    ...
    child.on('close', (code) => {
        if(code == 0) {
          ...
          res.status(200).json({outcome: 'Gateway Deleted'})
        } else {
          console.log("Error Deleting Gateway")
          res.status(200).json({outcome: 'Error Deleting Gateway'})
        }
      })
    } catch (err) {
      res.status(100).json({outcome: "Error Deleting Gateway"})
    }
} )
